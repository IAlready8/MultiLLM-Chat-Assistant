...
describe('native orchestration and explicit sidecar', () => {
  ...
  it('preserves structured sidecar failures without leaking upstream error text or redispatching', async () => {
    vi.stubEnv('PYTHON_CORE_URL', 'https://sidecar.example')
    vi.mocked(fetch).mockResolvedValue(Response.json([{ provider: 'openai', model: 'model-a', content: '', success: false, error: { code: 'RATE_LIMITED', message: 'private upstream body', retryable: true }, prompt_tokens: 0, completion_tokens: 0, cost_usd: 0, latency_ms: 3 }]))
    const response = await POST(request())
    const body = await response.json()
    expect(body).toMatchObject([{ success: false, status: 'failed', code: 'RATE_LIMITED', retryable: true, error: 'Provider rate limit reached, please retry shortly' }])
