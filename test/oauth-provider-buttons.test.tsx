...
describe('OAuthProviderButtons', () => {
  ...
  it('explains when no account-creation provider is configured', async () => {
    ...
    await waitFor(() => {
      expect(
        screen.getByText(
          /Contact the operator to enable a supported sign-in provider before creating an account/i,
        ),
      ).toBeInTheDocument()
    })
  ...
  it.each([null, new Error('offline')])('lets users retry failed provider discovery', async (failure) => {
    ...
    expect(screen.queryByText(/Contact the operator/i)).not.toBeInTheDocument()
