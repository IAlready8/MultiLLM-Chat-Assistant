...
try {
  ...
  const create = await request('/api/conversations', json({ title: 'Generation integration test', messages: [{ role: 'user', content: 'Compare this prompt', clientId: turnId }] }))
  assert.equal(create.status, 201)
  const conversation = await create.json()
  const payload = (model, position, requestId = randomUUID()) => ({ provider: 'ollama', model, conversationId: conversation.id, requestId, turnId, instanceId: `instance-${position}`, position, messages: [{ role: 'user', content: 'Compare this prompt' }] })
  ...
  for (let i = 0; i < 2; i++) assert.equal((await request(`/api/conversations/${conversation.id}`, { ...json(duplicateTurn), method: 'PUT' })).status, 200)
